...
class Q_Net(tf.keras.models.Model):
    def __init__(self, obs, dim1, dim2, dim3, action, loss, optimizer=None):
        super().__init__()

        self.model = tf.keras.Sequential([
            Dense(dim1, input_dim=obs, activation='relu'),
            Dense(dim2, activation='relu'),
            Dense(dim3, activation='relu'),
...
class ReplayBuffer:
    ...
    def sample_transition(self, batch_size=16):
        ...
        for index in random_indices:
            ...
            sampled_terminals.append(self.buffer[index][4])

        return (
            tf.convert_to_tensor(sampled_current_states),
            tf.convert_to_tensor(sampled_actions),
            tf.convert_to_tensor(sampled_rewards, tf.float32),
...
class DQN_LANDER:
    def __init__(self,obs, dim1, dim2, dim3, action, num_episodes, batch_size, loss_fn, learn_after_steps, target_update_after,
                 epsilon=1.0, epsilon_decay=1.005, gamma=0.99, max_transitions=100000, optimizer=None, verbose=1):
        super(DQN_LANDER, self).__init__()
        self.q_net = Q_Net(obs=obs, dim1=dim1, dim2=dim2, dim3=dim3, action=action, loss='mse', optimizer=optimizer)
        self.target_net = Q_Net(obs=obs, dim1=dim1, dim2=dim2, dim3=dim3, action=action, loss='mse', optimizer=optimizer)
