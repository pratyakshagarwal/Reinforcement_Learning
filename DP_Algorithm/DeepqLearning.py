...
hidden_state1 = Dense(64, activation='relu')(net_input)
hidden_state2 = Dense(32, activation='relu')(hidden_state1)
output = Dense(2, activation='linear')(hidden_state2)
...
i=0
state = CartEnv.reset()[0]
while i<20 and not done:
    random_states.append(state)
    state, _ , done, _, _ = CartEnv.step(policy(state).numpy())
    i+=1
...
for episode in range(NUM_EPISODES):
    ...
    while not done:
        ...
        next_state, _, done, _, _ = CartEnv.step(action.numpy())
        ...
        if step_counter % LEARN_AFTER_STEPS == 0:
            ...
            next_actions_values = tf.reduce_max(target_net(next_states), axis=1)
            targets = tf.where(terminals, rewards, rewards + GAMMA * next_actions_values)
    ...
    metric["lenght"].append(episode_lenght)
