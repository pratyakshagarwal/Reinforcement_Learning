...
#     def gather_random_states(self, env, policy, num_state=20):
#         random_states = []
#         done = False
#         i = 0
#         state = env.reset()[0]
#         while i < num_state and not done:
#             random_states.append(state)
#             action = policy.select_action(state).numpy()
#             state, _, done, _, _ =  env.step(action)
#             i += 1
#         return tf.convert_to_tensor(random_states)
...
class DQN_Agent:
    def __init__(self,obs, dim1, dim2, action, num_episodes, batch_size, loss_fn, learn_after_steps, target_update_after,
                 epsilon=1.0, epsilon_decay=1.005, gamma=0.99, max_transitions=100000, optimizer=None):
